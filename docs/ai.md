...
There are many use cases of AI technology in software development. These include code, test data and document generation in addition to very effective data analysis capabilities. I have found that the best results occur after taking time to clearly and comprehensively specify what the AI agent should do and not do. This helps to reduce bugs and omissions in code and to avoid expensive rework. Here are some real examples of how AI can be applied to software development.
...
Applying AI to software testing can have impressive results. The two SQL [queries](../code/dv.sql) below should return exactly the same results. But the first query incorrectly returns 0 rows while the second returns one row as expected. The only difference between the queries is that the second one uses an optimizer hint. But optimizer settings should not alter query results. This bug was detected by an AI generated program that is capable of validating SQL query results.
